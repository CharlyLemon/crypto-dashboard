...
def fetch_klines(symbol: str, interval: str, limit: int = 200) -> pd.DataFrame:
    """Fetch OHLCV — tries Bybit first (works everywhere), then Binance Futures, then Binance Spot."""
    # 1. Bybit — no geo-restrictions, works from Streamlit Cloud (GCP/AWS)
    try:
        ...
        pass
    # 2. Binance Futures (fallback for local/residential environments)
    try:
        r = requests.get(
...
def fetch_ticker(symbol: str) -> dict:
    """Fetch 24h ticker — tries Bybit first, then Binance Futures, then Binance Spot."""
    # 1. Bybit
    try:
        bybit_sym = BYBIT_SYMBOL_MAP.get(symbol, symbol)
        r = requests.get(
...
def fetch_funding_rate(symbol: str) -> float:
    """Fetch latest funding rate — tries Bybit first, then Binance Futures."""
    # 1. Bybit
    try:
        bybit_sym = BYBIT_SYMBOL_MAP.get(symbol, symbol)
        r = requests.get(
...
def fetch_open_interest(symbol: str) -> float:
    """Fetch open interest — tries Bybit first, then Binance Futures."""
    # 1. Bybit
    try:
        bybit_sym = BYBIT_SYMBOL_MAP.get(symbol, symbol)
        r = requests.get(
...
<div style='text-align:center;padding:20px;color:#475569;font-size:10px;border-top:1px solid #1f2d40;margin-top:24px'>
    ⚡ CryptoPerp Dashboard · Datos: Bybit / Binance Futures / Binance Spot + CoinGecko (free tier) + alternative.me<br>
...
# Must be at the END so all content renders before the rerun is triggered.
# ─────────────────────────────────────────────
if auto_refresh:
