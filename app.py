...
# ─────────────────────────────────────────────
# API FUNCTIONS  (Futures → Spot → CoinGecko fallback)
# ─────────────────────────────────────────────
...
def fetch_klines(symbol: str, interval: str, limit: int = 200) -> pd.DataFrame:
    """Fetch OHLCV — tries Futures, then Spot."""
    params = {"symbol": symbol, "interval": interval, "limit": limit}
    # 1. Binance Futures
    try:
        r = requests.get(f"{BINANCE_FUTURES}/fapi/v1/klines", params=params, timeout=10)
    ...
    try:
        r = requests.get(f"{BINANCE_SPOT}/api/v3/klines", params=params, timeout=10)
        if r.status_code == 200:
            data = r.json()
            if isinstance(data, list) and len(data) > 10:
                return _parse_klines(data)
...
def fetch_ticker(symbol: str) -> dict:
    """Fetch 24h ticker — tries Futures, then Spot."""
    # 1. Futures
    try:
...
def fetch_funding_rate(symbol: str) -> float:
    """Fetch latest funding rate from Futures (no fallback — spot doesn't have it)."""
    try:
...
def fetch_open_interest(symbol: str) -> float:
    """Fetch open interest from Futures."""
    try:
...
def evaluate_signals(ind: dict, funding: float, oi: float, fg: dict) -> dict:
    """
    Score each indicator +1 (bullish), -1 (bearish), 0 (neutral).
    Confluence ≥ 3 bullish → LONG
    Confluence ≥ 3 bearish → SHORT
    """
    if not ind:
        return {}
...
with st.spinner("Cargando datos de mercado..."):
    ...
    signals = evaluate_signals(ind, funding, 0, fg)
...
fg_ok      = fg.get("value", 50) != 50 or fg.get("label") != "Neutral"
...
if not klines_ok or not ticker_ok:
    with st.expander("⚠️ Diagnóstico de conexión API", expanded=True):
        ...
        cols_d[2].markdown(status_badge(True,        "Funding Rate"),    unsafe_allow_html=True)
        cols_d[3].markdown(status_badge(True,        "Fear & Greed"),    unsafe_allow_html=True)

        if not klines_ok:
            st.warning(
                "No se pudo conectar a Binance (Futures ni Spot). "
                "Streamlit Cloud puede tener bloqueado el acceso. "
...
metrics = [
    ...
    (m6, "EMA 200", fmt_price(ind.get("ema200", 0)) if ind else "—", "tendencia macro"),
]
