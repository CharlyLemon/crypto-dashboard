...
def build_chart(df, ind):
    ...
    for p, color in [("20","#F5A623"), ("50","#60A5FA"), ("200","#C084FC")]:
        ...
        if key in ind:
            fig.add_trace(go.Scatter(x=t, y=ind[key], name=f"EMA{p}",
                ...
                hovertemplate=f"EMA{p}: %{{y:,.4f}}<extra></extra>"), row=1, col=1)
    ...
    fig.add_trace(go.Bar(x=t, y=ind["_macd_hist"], marker_color=hist_colors, showlegend=False,
        hovertemplate="Hist: %{y:.5f}<extra></extra>"), row=3, col=1)
    fig.add_trace(go.Scatter(x=t, y=ind["_macd"], name="MACD",
        line=dict(color="#2DFF6E", width=1.5),
        hovertemplate="MACD: %{y:.5f}<extra></extra>"), row=3, col=1)
    fig.add_trace(go.Scatter(x=t, y=ind["_macd_signal"], name="Signal",
        line=dict(color="#F5A623", width=1.5),
        hovertemplate="Signal: %{y:.5f}<extra></extra>"), row=3, col=1)
...
with st.expander("Ver tabla de señales rápidas — 20 pares · 4h", expanded=False):
    ...
    for i, (lbl, okx_id) in enumerate(CRYPTOS.items()):
        ...
        try:
            ...
            _signal_icon = {"LONG": "▲ LONG", "SHORT": "▼ SHORT", "NEUTRAL": "◆ NEUTRAL"}
            rows.append({
                "Par": lbl,
                "Precio": fmt_price(ind_t.get("price",0)) if ind_t else "—",
                "24h %": f"{chg_t:+.2f}%",
                "RSI": round(ind_t.get("rsi", 0), 1) if ind_t else None,
                "Funding": f"{fr_t:+.4f}%",
                "Bull": bc, "Bear": brc,
                "Señal": _signal_icon.get(act_t, act_t),
            })
        except Exception:
            rows.append({"Par":lbl,"Precio":"—","24h %":"—","RSI":None,"Funding":"—","Bull":0,"Bear":0,"Señal":"✕ ERROR"})
        time.sleep(0.05)
    prog.empty()
    if rows:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Bull": st.column_config.ProgressColumn(
                    "▲ Bull", min_value=0, max_value=9, format="%d"
                ),
                "Bear": st.column_config.ProgressColumn(
                    "▼ Bear", min_value=0, max_value=9, format="%d"
                ),
            },
